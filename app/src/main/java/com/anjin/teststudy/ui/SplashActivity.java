...
import com.anjin.teststudy.databinding.ActivitySplashBinding;
...
import com.anjin.teststudy.viewmodel.SplashViewModel;
...
public class SplashActivity extends NetworkActivity<ActivitySplashBinding> {
    ...
    protected void onObserveData() {
        //城市数据返回
        viewModel.getListMutableLiveData().observe(this, provinces -> {
            {
                if (provinces.size() == 0) {
                    Log.d(TAG, "onObserveData: 第一次添加数据");
                    //没有保存过数据，只需要保存一次即可。
                    List<Province> provinceList = loadCityData();
                    if (provinceList != null) {
                        viewModel.addCityData(provinceList);
                    }
                } else {
        ...
        //必应壁纸数据返回
        viewModel.getBingMutableLiveData().observe(this, bingResponse -> {
        ...
        viewModel.getFailed().observe(this, this::showLongMsg);
    }
