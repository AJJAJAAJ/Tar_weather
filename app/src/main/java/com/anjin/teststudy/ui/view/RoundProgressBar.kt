...
class RoundProgressBar : View {
    ...
    private var mArcBgColor //圆弧背景颜色
            = 0
    private var mMaxProgress //最大的进度，用于计算进度与夹角的比例
            = 0f
    private var mCurrentAngleSize //当前进度对应的起点角度到当前进度角度夹角的大小
            = 0f
    private var mCurrentProgress = 0f //当前进度
    private var mDuration: Long = 2000 //动画的执行时长
    private var mProgressColor //进度圆弧的颜色
            = 0
    ...
    private fun initAttr(context: Context, attrs: AttributeSet?) {
        val array = context.obtainStyledAttributes(attrs, R.styleable.RoundProgressBar)
        ...
        mStrokeWidth =
            dp2px(array.getDimension(R.styleable.RoundProgressBar_round_stroke_width, 12f))
        mCurrentProgress = array.getFloat(R.styleable.RoundProgressBar_round_progress, 300f)
        mProgressColor =
            array.getColor(R.styleable.RoundProgressBar_round_progress_color, Color.RED)
        ...
        mFirstTextColor =
            array.getColor(R.styleable.RoundProgressBar_round_first_text_color, Color.RED)
        ...
        mSecondTextColor =
            array.getColor(R.styleable.RoundProgressBar_round_second_text_color, Color.RED)
     ...
     * @param centerX 文本
     */
    private fun drawSecondText(canvas: Canvas, centerX: Float) {
        val paint = Paint()
        paint.isAntiAlias = true
     ...
     * @param bottomY Y轴位置
     */
    private fun drawMinText(canvas: Canvas, leftX: Float, bottomY: Float) {
        val paint = Paint()
        paint.isAntiAlias = true
     ...
     * @param bottomY Y轴位置
     */
    private fun drawMaxText(canvas: Canvas, rightX: Float, bottomY: Float) {
        val paint = Paint()
        paint.isAntiAlias = true
