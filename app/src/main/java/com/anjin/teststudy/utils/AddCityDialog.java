...
import com.anjin.teststudy.databinding.DialogAddCityBinding;
import com.anjin.teststudy.datebase.bean.SearchCityResponse;
import com.anjin.teststudy.repository.SearchCityRepository;
import com.anjin.teststudy.ui.adapter.RecommendCityAdapter;
import com.anjin.teststudy.ui.adapter.SearchCityAdapter;
    ...
    public static void show(Context context, List<String> cities, SelectedCityCallback selectedCityCallback) {
        ...
        binding.etSearchCity.setOnEditorActionListener((v, actionId, event) -> {
            if (actionId == EditorInfo.IME_ACTION_SEARCH) {
                ...
                searchCityAdapter.changeTxColor(cityName);
                BottomSheetBehavior.from(Objects.requireNonNull(dialog.findViewById(com.google.android.material.R.id.design_bottom_sheet)))
                        .setState(BottomSheetBehavior.STATE_EXPANDED);
                //搜索城市
                SearchCityRepository.Companion.getInstance().searchCity(searchCityResponseMutableLiveData, failed, cityName, true);
                //关闭输入法
